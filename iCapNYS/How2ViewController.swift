...
class How2ViewController: UIViewController {
  ...
  //      scrollView.bringSubviewToFront(textViewOnScrollView)
    ...
    func addTextEn(){
        ...
        textViewOnScrollView.text.append("Select the camera you want to use with the green button in the bottom right, and adjust settings like zoom and exposure. Long tap on camera select button is available. An explanatory video will play if you choose “with Video Auto90s”,  showing how to record, allowing you to follow along. You can also choose the Unimec WiFi camera; in that case, you will need to set your iPhone's WiFi to the Unimec SSID. During recording, a large stop button (which is faint and hard to see) appears in the center of the screen, and tapping it will stop the recording.\n\n")
    ...
    func addTextJa(){
        ...
        textViewOnScrollView.text.append("右下の緑色ボタンで使用するカメラを選択し、ズーム、露出などを設定します。カメラ選択ボタンの長押しでは前のカメラを選択できます。")
        textViewOnScrollView.text.append("解説動画付自動90秒では撮影方法の説明映像が流れ、説明を聞きながら録画できます。録画中は画面中央に大きな半透明の薄い色のストップボタンがあり、それをタップすると録画終了します。Unimec社のWiFiカメラも選択できます。その場合は、iPhoneのWiFiをUnimec社のSSIDに設定する必要があります。\n\n")
